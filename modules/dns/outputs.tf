...
output "health_check_arn" {
  ...
  value       = try(aws_route53_health_check.alb[0].arn, "")
}

output "primary_record_fqdn" {
  description = "Primary domain A record FQDN"
  value       = try(aws_route53_record.alb_alias[0].fqdn, "")
}
...
output "dns_records_summary" {
  ...
  value = {
    ...
    cdn_subdomain   = local.cdn_record_enabled ? "${var.cdn_subdomain}.${var.domain_name} -> ${var.cloudfront_domain_name}" : "Not created"
  }
}
