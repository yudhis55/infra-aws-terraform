...
jq -n --arg script "$script_payload" --arg profile "$profile_payload" '{commands:[
  ...
  "docker run --rm -e APP_URL -e EXPERIMENT_ID -e TRIAL_ID -e BASELINE_VUS -e STEP_1_VUS -e STEP_2_VUS -e STEP_3_VUS -e WARMUP_DURATION -e BASELINE_DURATION -e STEP_1_DURATION -e STEP_2_DURATION -e STEP_3_DURATION -e RECOVERY_DURATION -v /tmp:/scripts grafana/k6@sha256:e7eeddf1ce2361df6920d925297f487c0ba549c44be242c6a9c22f28d9b08efa run --summary-export /scripts/eepistore-k6-summary.json /scripts/eepistore-load.js",
